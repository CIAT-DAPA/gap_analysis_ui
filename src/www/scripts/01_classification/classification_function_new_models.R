                               ...
                               external_df     = NULL)
{
  ...
  model_list <- caretEnsemble::caretList(
    ...
    tuneList   = list(ranger = caretEnsemble::caretModelSpec(method = "ranger", importance = "impurity")),
  ...
  impVar_list <- lapply(1:length(model_list), function(i){
    try( vImportance <- caret::varImp(object = model_list[[i]]))
    
    if(is.null(vImportance)){
      impVar <- data.frame(impVar = NA)
    }else{
