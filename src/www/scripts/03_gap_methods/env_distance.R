...
calc_env_score <- function(sdm_path, 
  ...
  if(!file.exists(paste(out_dir, "/environmental_score.tif", sep = ""))){
    ...
    sdm_prj <- raster(sdm_path) 
    sdm_prj <- readAll(sdm_prj)
    
    #load accessions
  
      occ_data <- shapefile(paste0(occ_dir, "/occurrences.shp"))
      occ_data <- unique(as.data.frame(occ_data)) %>% 
        dplyr::select(Y, x,y)
    ...
    if(!file.exists(paste(gap_dir,"/ecogeo_",clus_method,".tif",sep=""))){
      ...
      clus_rs <- ecogeo_clustering(n.sample = n.sample, 
                                   var_names = var_names, 
                                   k.clust = n.clust,
                                   sdm_path = sdm_path,
                                   climDir = env_dir,
    ...
    rs_euc_norm[which(is.na(rs_euc_norm[]) & !is.na(SDM[]))] <- max(rs_euc_norm[],na.rm=T)
    
    rs_euc_norm <- rs_euc_norm*SDM
    
    #write output
    #writeRaster(rs_euc, paste(out_dir,"/euclidean_dist_",clus_method,".tif",sep=""),format="GTiff", overwrite = TRUE)
    writeRaster(rs_euc_norm, paste(out_dir,"/environmental_score.tif",sep=""),format="GTiff", overwrite = TRUE)
    #return rasters
    #return(rs_euc_norm)
